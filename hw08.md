...
异：管理学 WBS 强调项目划分的完整性和互斥性，忽视具体执行；信息学 Top-down 着重功能的细化与实现
...
until (get_water_volume() > StandardHeight)
...
rollerrunning() {
    ...
    for RunFrequency from 1 to n
    repeat
        motor_run(left)
    until (time_counter() > StandardRunTime)
    repeat
        motor_run(right)
    until (time_counter() > StandardRunTime)
    motor_run(stop)
